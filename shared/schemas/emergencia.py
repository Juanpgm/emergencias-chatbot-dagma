...
class TipoEmergencia(str, Enum):
    ...
    rescate_animales_silvestres = "rescate_animales_silvestres"
    tala_arboles = "tala_arboles"
    contaminacion_fuente_hidrica = "contaminacion_fuente_hidrica"
...
class DatosEmergencia(BaseModel):
    nombre_reportante: Optional[str] = Field(None, description="Nombre completo de la persona que reporta.")
    ...
    direccion_hechos: Optional[str] = Field(None, description="Dirección donde ocurrieron los hechos.")
    direccion_persona: Optional[str] = Field(None, description="Dirección de residencia de quien reporta.")
    tipo_de_emergencia: TipoEmergencia = Field(
        ...,
        description="Tipo de emergencia ambiental. arbol_caido, rescate_animales_silvestres, tala_arboles, contaminacion_fuente_hidrica.",
    )
    descripcion_emergencia: str = Field(..., description="Descripción breve del incidente (máx. 2 oraciones).")
    descripcion_detallada: str = Field(..., description="Resumen ampliado con contexto adicional inferido.")
    ubicacion_inferida: Optional[str] = Field(None, description="Ubicación inferida a partir del mensaje (barrio, comuna, punto de referencia).")
    ...
    nivel_de_gravedad: NivelGravedad = Field(..., description="Nivel de gravedad: alta, media o baja.")
    requiere_atencion_inmediata: bool = Field(..., description="True si la situación requiere atención inmediata.")


class DatosContacto(BaseModel):
    nombre_reportante: Optional[str] = Field(None, description="Nombre completo de la persona que reporta.")
    telefono: Optional[str] = Field(None, description="Número de teléfono colombiano de contacto (10 dígitos, empieza por 3).")


class DatosUbicacion(BaseModel):
    direccion_hechos: Optional[str] = Field(None, description="Dirección exacta donde ocurrieron los hechos (calle, carrera, número).")
    ubicacion_inferida: Optional[str] = Field(None, description="Ubicación inferida: barrio, comuna o punto de referencia en Cali.")


class WhatsAppMessage(BaseModel):
    message_sid: Optional[str] = Field(None, alias="MessageSid")
    from_number: str = Field(..., alias="From", description="Número del remitente (ej. whatsapp:+573001234567).")
